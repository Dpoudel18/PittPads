...
manual_price_min = st.sidebar.text_input("Minimum Budget", 0)  # Set default as slider's current min
manual_price_max = st.sidebar.text_input("Max Budget", int(df['price'].max()))  # Set default as slider's current max
...
# Apply the price filter based on either slider or manual input
if manual_price_min > manual_price_max:
    st.sidebar.error("Min price cannot be greater than max price")
else:
...
# Display the map in Streamlit (optional)
import streamlit as st
