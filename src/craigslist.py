...
base_url = "https://pittsburgh.craigslist.org/search/apa?hasPic=1#search=1~gallery~{page}~0"
...
with open('craigslist_apartments_final.csv', mode='w', newline='', encoding='utf-8') as file:
    ...
    writer.writerow(['Name', 'Type', 'Price', 'Neighborhood', 'Address', 'Latitude', 'Longitude', 
                     ...
                     'Parking', 'Image Link','Link to Listing'])
    ...
    while listings_found:
        ...
        url = base_url.format(page=page_number)  # Update to use the gallery logic
        ...
        for listing in listings:
            ...
            image_url = image_tag.get('src') if image_tag else 'No image available'

            # Write the data into the CSV file
            writer.writerow([name, apartment_type, price, neighborhood, address, latitude, longitude, 
                             bedrooms, bathrooms, sq_ft, rent_period, laundry, parking, image_url, link])
...
df_cleaned = pd.DataFrame({
    ...
    'units': None,
    'phone': None,
    'image_url': df['Image Link'].fillna(''),
...
df_cleaned = df_cleaned.dropna(subset=['price', 'name', 'location', 'zip_code', 'property_url']).drop_duplicates(subset='name', keep='first')
